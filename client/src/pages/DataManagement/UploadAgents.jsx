...
import { useUploadAgentsMutation } from '../../store/apiSlice'
...
export default function UploadAgents() {
  const [upload] = useUploadAgentsMutation()
    ...
    <div className="font-[Inter]">
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Upload a CSV file with agent data. Agents are stored in{' '}
          <code className="text-xs">ins_agents</code> and linked to channels,
          regions, and the MLM hierarchy.
        </p>
      </div>
      ...
      <div className="mb-6">
        <p className="text-xs text-gray-400">
          CSV must include: agent_code, agent_name, channel_code, region_code,
          branch_code, license_number, license_expiry, activation_date,
          parent_agent_code, hierarchy_level, status
        </p>
      </div>
      ...
      <div className="mb-6 flex items-center gap-3">
        {!uploadDone ? (
          ...
          >
            {isUploading ? 'Uploading…' : 'Upload Agents'}
          </button>
        ) : (
          <>
            <span className="text-sm font-medium text-green-600">
              ✓ {uploadCount} agent(s) uploaded
            </span>
            <button
              type="button"
              onClick={handleReset}
      ...
      {uploadDone && agentCodes.length > 0 && (
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          ...
          <p className="mt-3 text-xs text-gray-400">
            Agents are now available for assignment and incentive calculations.
          </p>
        </div>
      )}
    </div>
