...
const EXPECTED_HEADERS = [
  'policy_number', 'agent_code', 'product_code', 'channel_code',
  'region_code', 'transaction_type', 'policy_year', 'premium_amount',
  'sum_assured', 'annualized_premium', 'payment_mode', 'issue_date',
  'due_date', 'paid_date', 'policy_status',
]

export default function UploadPolicyTransactions() {
  const { data: programs = [] } = useGetProgramsQuery()
  const [upload] = useUploadPolicyTransactionsMutation()
  ...
  const handleUpload = async () => {
    if (!file || !programId) {
      setError('Please select a program and a CSV file.')
      return
    }
    setError('')
    setIsUploading(true)
    ...
    try {
      ...
      const codes = [...new Set(result.rows.map((r) => r.agent_code))]
    ...
    <div className="font-[Inter]">
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Upload a CSV file with policy transaction data. Transactions are
          stored in <code className="text-xs">ins_policy_transactions</code> and
          used by the incentive calculation engine.
        </p>
      </div>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="flex items-end">
          <p className="text-xs text-gray-400">
            CSV must include: policy_number, agent_code, product_code,
            channel_code, region_code, transaction_type, policy_year,
            premium_amount, sum_assured, annualized_premium, payment_mode,
            issue_date, due_date, paid_date, policy_status
          </p>
        </div>
      </div>
        ...
        {!uploadDone ? (
          ...
          <>
            <span className="text-sm font-medium text-green-600">
              ✓ {uploadCount} transactions uploaded for{' '}
              {agentCodes.length} agent(s)
            </span>
            <button
              type="button"
              onClick={handleReset}
      ...
      {/* ── Uploaded Agent Summary ──────────────────────────────────────── */}
      {uploadDone && agentCodes.length > 0 && (
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          ...
          <p className="mt-3 text-xs text-gray-400">
            Run the incentive calculation from the Calculate menu to process
            these transactions.
          </p>
        </div>
      )}
