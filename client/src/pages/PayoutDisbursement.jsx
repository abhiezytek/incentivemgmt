  ...
  return (
    <div className="font-[Inter]">
      ...
      <div className="mb-4 flex flex-wrap gap-3">
        ...
        <button
          ...
          onClick={() => {
            ...
            bulkAction(
              ...
              { programId: filters.programId, periodStart: ps },
              'paid',
              (d) => `💸 ${d.paid ?? d.paidCount ?? 0} result(s) marked paid`,
            );
          }}
        ...
        <button
          type="button"
          disabled={actionLoading === 'initiate' || selected.length === 0}
          onClick={() =>
            bulkAction(
              'initiate-payment',
              { ids: selected },
              'initiate',
              (d) => `🏦 ${d.count ?? 0} result(s) payment initiated`,
            )
          }
          className="rounded-md border border-yellow-400 px-4 py-2 text-sm font-medium text-yellow-700 hover:bg-yellow-50 disabled:opacity-40"
        >
...
function SingleActionButton({ agent, onDone, API }) {
  const next = {
    ...
    APPROVED: {
      ...
      body: { ids: [agent.id] },
      style: 'text-yellow-700 border-yellow-300 hover:bg-yellow-50',
    },
    INITIATED: {
      label: 'Mark Paid',
      endpoint: 'mark-paid',
